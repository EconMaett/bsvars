...
title:
  formatted: "Fast and Efficient Bayesian Estimation of Structural VARs Using the \\proglang{R} Package \\pkg{bsvars} version 2.1.0."
...
1. Specify the model using one of the `specify_*` functions for the demanded model
2. Estimate the model using the `estimate` method
3. Analyse the posterior output by computing impulse responses using function \linebreak
`compute_impulse_responses()`, forecast error variance decomposition using function `compute_variance_decompositions()`
4. Provide the plots and tabular representations.
...
S         = 10000
set.seed(123)
burn_in   = estimate(sp_bsvar, S = Sbi, show_progress = FALSE)
es_bsvar  = estimate(burn_in, S = S, thin = 1)
```
...
```
In order to investigate whether the normalisation was successful a trace plot of the first column of matrix $\mathbf{B}$ is reported.
```{r check normalisation, fig.cap = "MCMC sample for the first column of the structural matrix", echo = FALSE}
plot.ts(t(es_bsvar$posterior$B[,1,]), bty = "n", col = "deeppink2", main = "")
```
The trace plot indicates a well-normalised sample as the symmetry around point zero has been removed. Consequently, the posterior sample is suitable for analysis and sample statistics, such as the mean reported below, can be computed and interpreted.
```{r Bmean}
...
```
```{r estimateSV2, cache=TRUE}
burn_in       = estimate(sp_bsvar_sv, S = Sbi, show_progress = FALSE)
es_bsvar_sv   = estimate(burn_in, S = S, thin = 1, FALSE)
...
```
```{r estimateMS2, cache=TRUE}
burn_in       = estimate(sp_bsvar_ms, S = Sbi, show_progress = FALSE)
es_bsvar_ms   = estimate(burn_in, S = S, thin = 1, FALSE)
